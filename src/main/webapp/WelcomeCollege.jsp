...
<div class="container-fluid">
  <div class="row" style=margin:20px>
  	...
  	<div class="col-sm-6">
		  <div class="container shadow-lg p-4 mb-4 bg-white rounded" style="height:670px" >
		      <h3>Reviews & Ratings</h3><hr>
		       <div style="height:600px;overflow-y:auto;width: 100%;">
		      <%
		      ArrayList<Review> reviews=(ArrayList<Review>)request.getAttribute("reviews");
		      for(Review r:reviews)
		      {  
		      %>   <div class="container" style="padding-bottom:20px;margin-bottom:50px;">
		          <table class="table table-striped">
		          <tbody> 
				      <tr>
				        <td>Email : <%=r.getEmail() %></td>
				        <td>Ratings: <%=r.getRate() %></td>				        
				      </tr>
				      <tr>
				         <td>Subject : <%=r.getSubject() %></td>
				        <td>Test Id : <%=r.getTestId()%></td>			        
				      </tr>
				      <tr>
				      <td>Review : <%=r.getReview() %></td>	
				      <td> </td>				     
				      </tr>
				  ...
				  <div class="form-group">
				    <label for="resp">Respone:</label>
				    <input type="text" class="form-control" placeholder="Type response" name="resp" >
				    <input type="hidden" name="rid" value="<%=r.getRid()%>">
				  </div>
				  <button type="submit" class="btn btn-success float-right" name="task" value="addResponse">Response</button>
				  </form>
				  </div>
				  </div>
				  <hr>
